...
| 日付       | バージョン | 説明                                            |
| ---------- | --------- | ------------------------------------------------ |
| 2026/04/16 | 0.14.0    | ベースイメージ更新                               |
| 2026/03/29 | 0.13.0    | Dependabot導入(依存関係の自動更新)               |
...
## 設計方針
- 本テンプレートは以下の想定ユースケースから Kubernetes Cronjob を念頭に設計・構成
- 本テンプレートは以下の理由からリアルタイム処理を採用していない

想定ユースケース:
- ECモールの注文連携・在庫連携などの定期バッチ処理
...
## 依存関係管理 (Dependabot)
- Dependabotにより依存関係の自動更新を実施
- requirements ディレクトリ配下の依存ファイルを対象に更新を検出
- 新しいバージョンが存在する場合、自動でPull Requestを作成
- セキュリティ脆弱性が検出された場合は優先的に修正PRが作成される

### 運用ルール
- DependabotのPRは通常のIssue駆動開発とは別扱いとする
- CIが成功していることを確認した上でマージする
- CIが失敗している場合はマージしない
...
### 開発者向け注意点
- DependabotによるPRマージ後は、ローカル環境を最新化すること

```bash
## ローカル環境のブランチをmainブランチに切替
$ git checkout main
